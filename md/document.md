...
現在、コンピューターで音を合成する方式は電話を始めとした音声通信の分野で発展したPCM(パルス符号変調)方式に起源を見ることができる。PCM方式とは音圧信号を一定の時間間隔で区切り(離散化)数列にし、その値である音圧を一定の間隔で分割し(量子化)、バイナリデータとして扱う表現方法である[@Taniguchi2015]。
