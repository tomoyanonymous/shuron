...
インプットやユーザーインターフェース（音程）の操作は経てば風で操作されたり、モビールのようにオブジェクトが自律的に動くことで変化したりするようになっていて、人を介さずに音を発音すると同時に、人の介入によってもパラメーターが変化する部分的なインタラクションを含む。
...
（入る場所がないな）**展示＝演奏というメタファー**（大友良英、音楽と美術のあいだ[@Otomo2017]）

あと、人が演奏するパラメータ部分を自動化しているところの説明が特に無い（毛利さん三原さんのヴェクサシオンの話（大友さん本）とかも使えそうだけど）
...
こうした流れは物理モデリングの非リアルな応用の理由とティンカリング的側面で重なるところもあるし、それをより俯瞰したプロセスでもあったと位置づけられる。
...
## 考察
開発者から見た視点でのExidiophoneの楽器としての特徴は最終的に3つの問題に集約された。
...
こうした議論を踏まえると、音を計算機を用いて生成するときのフォーマットに万能と言えるものは存在せず、折れ線形式のようにある特定の波形を表現するには
...
Ralf Baeckerは5つの相互接続された信号処理、信号生成モジュールを水晶やシリコンといった今日のコンピューターを構成する基本的素材をそのまま用いることで高度にブラックボックス化されてしまったコンピューターにおけるプロセスを材料レベルまで還元して提示する作品『Irrational Computing』(2011)を発表している[@Baecker2011]。
...
![edtac-wip1](../img/edtac-wip1.jpg){#fig:edtac-sketch width=70%}

![edtac-wip2](../img/edtac-wip2.jpg){#fig:edtac-structure width=70%}
...
![カウンター側制作の様子。Illustrator上で作成した図面を実寸で出力して針金それに合わせて切り出した。](../img/edtac-wip3.jpg){#fig:edtac-wip3 width=50%}

![タイマー側制作の様子。](../img/edtac-wip4.jpg){#fig:edtac-wip4 width=50%}
...
それでも任意の時間シーケンスを再現性を持って再現できるという点では高橋のニューマチック・コンピューターと状況は近い。加えて、世界で最初の電子計算機と呼ばれるENIACの構造はデータフロー方式といって、ケーブルで個別の計算ユニットをつなぎ替えることによってプログラミングを行っている。Wendy Hui Kyong Chunが*「a new ENIAC was created each time it was used (使われるたびに新しいENIACが作られていたのだ)」*と表現する[@Chun2005,28p]ように、回路の組み換えをプログラミングということはできる。Chunはまた、何をもってプログラミングが達成されたか、という問題は実は曖昧であることを指摘する。
...
抽象化するためには言語化が必要、シンボリズムが必要

「機能は達成できるけどシンボリズムに頼らない」というのが計算機として特徴だと言える

その上で時間のシンボル化はそもそもできるのかみたいな問題はまだ解決されないところだろう（あんまり哲学の方向に踏み込まず、さらっと）
...
『Aphysical Unmodeling Instrument』ではWhirlwindという特異な抽象度を持った物理モデルを再物理化して展示するというプロセスを繰り返した。その中で物理モデルとはただの計算式ではなく開発者がどんな意図でその実装を行ったのかという情報を含めたものだといえ、ある種楽譜のようなものに近い曖昧さを含んでいる事がわかってきた。VL1やCORDIS-ANIMAといった非リアルな物理モデリング
...
『EDTAC』では抵抗値などの物理的な値で時間のシーケンスを制御できるマスタークロックを持たないプリミティブなコンピュータを制作した。ソフトウェアの定義の曖昧さに触れることで、任意の命令を実現できているものの、完全に言語的な命令に収まりきらない機能を実現しているこの機械の制作から、音をプログラミングすることの最小単位は実際には存在せず、演奏指示をする形での作曲行為や、特定の音が出るオブジェクトをつくる楽器制作という行為のあいだのグラデーションを見せることになる。

サンプリング理論に基づく音合成への不満を示した[@puckette2015sampling]の議論がこの3つの考察全てに関わってくるので、それを改めて見直そう。Pucketteは4章背景で述べたとおりサンプリング理論に基づく音合成の限界を示し、そうではない音合成として直線の組み合わせによる記述法などを挙げているが、他にも物理モデリングを出発点として、特定の楽器を模倣するのではない入出力を持った抽象的な関数の組み合わせとして音を記述する方法を挙げる。例えば、空間を重力に従って動き回る点の軌跡を音圧として書くようなシステムだが、空間にワームホールのようなおかしなつながりを導入したりするようなものを例示する。彼は議論のまとめとして、様々な物理モデリングの発展にもかかわらず未だ現実の楽器にはコンピューターで模倣しきれない表現力があり、その理由はおそらくはリアルタイムでのインタラクションの少なさと、物理的なプロセスが持つ複雑性と予測不可能性にあるとする。そのうえで彼は最後に、こうした予測不可能さは既にCrackleboxなどのメインストリームでない電子音楽で探求が古くから成されておりその重要さを指摘している。

この議論に付け加えるのであれば、そうした限界が現れてきた理由には人間の聴覚に基づいて音を記述してきた歴史にあるだろう。聞こえの音の高さやリズムに基づいて記述してきた五線譜に始まり、人間の知覚に基づいて音を再現するPCMまで、知覚に基づいた音の記述には、暗に自分が出した音が自分の聞こえと一致していないければならないという制御の欲望があらわれている。

そうして制御できないシステムをつくる入り口として物理モデリングのようなものに発想を得て、もう一度リアルでないシステムに戻っていく。Collinsはこの様なプロセスをErrantー逸脱した、あるいは家出の、迷子の―Sound Synthesisと名付けているのは非常に納得がいく[@Collins2008]。これはサーキットベンディングや失敗の美学のような技術の誤った使い方とは、技術をきちんと掘り下げて理解した上で更に間違える、という点で大きく異なる。

Aphyisical Unmodeling Instrumentはそうした問題を音合成の手法としてではなく、モデリングとその解釈のプロセスをインスタレーションの制作という形で提示するものであった。Exidiophoneはこうした制御不可能性を出発点にしつつ、その上でまだ残るコンピューターで記述不可能なシステムの存在を示唆する。



（AUIとExidiophone、抽象的なシステムのアイデンティティが確立するための様子）
